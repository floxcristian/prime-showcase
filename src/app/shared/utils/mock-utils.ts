 ...
 *
 * **Promoción a shared (RFC-002 D8):** estos helpers nacieron en
 * `modules/observability/mocks/mock-utils.ts` y se MOVIERON acá — no se
 * duplicaron ni se importan cross-módulo. Un import
 * `modules/social → modules/observability` acoplaría dos features
 * hermanas (nadie esperaría que borrar observability rompa social);
 * duplicar violaría la regla #1 del repo. El archivo original queda como
 * re-export de una línea, manteniendo los imports existentes de
 * observability intactos (cero churn). Precedente de ubicación:
 * `shared/utils/mock-latency.ts`. `dailySeries` NO se promueve: es
 * vocabulario del dominio social y vive en `mocks/social-mock-utils.ts`.
 *
